...
| `process_media` | Computes a record's context now through the platform's processor. Router role only, and it **costs the organization's credits**: use it only when the record has no context. Your own model is free; the platform's processor is not. |
