...
- `/bowerbirds-route <bucket>` runs one routing cycle as the **external router** of the router supervising that bucket — the slug is how the router is found, and a router may supervise several buckets. It claims the ROUTER's cycle (one claim, covering every bucket it supervises), reads the routes, the router's global filters and the bucket list, then works each bucket it can read in turn: fetches the unsigned records, reads each record's findings (`get_record_context`: the summary and the findings with their chunk references, `get_media_chunk` when one finding needs the chunk behind it), searches that bucket for what already exists (`search_records`, `get_record`, and the route's connector read tools), decides per record per route (merge, derive, create, append, link or none), acts only through `derive_record` and the route's enabled connector tools, signs that bucket's records with their statuses, links and the finding ids each rested on — once per bucket, under the same cycle — and releases the cycle once at the end. It never moves or trashes a record, and a clamp from the gate stops that record and is reported verbatim.
...
## Routing your own buckets from your own agent

A bucket has at most one router; a router may watch several buckets. Make it an external one whose brain is your agent: mint a workspace token with the `router` role (`bowerbirds cloud tokens mint --role router`, `bowerbirds cloud tokens login --role router` through the browser approval page, or Developer Tools in the app), attach a router of kind `external` naming that token and every bucket it should watch (`bowerbirds cloud routers create --kind external --token <id> --name <name> --bucket <owner/slug>`, repeating `--bucket` per bucket, with `--path <bucket>:<path>` to narrow one of them and `--filter kind=…|keyword=…|ageDays=…` for the router's global filters), and give it routes — destinations, linked connectors with per-tool grants, a description and rules; the **Bugs to Linear** template is one tap. Then `/bowerbirds-route <bucket>` from your terminal — any one of its buckets — is the cycle over all of them: your model, your tools, zero platform credits, and the records end up with the same route marks, router status and destination links a native router writes. The router role can add and sign only; the move and trash doors refuse it.

The five routing tools, advertised to a router token beside the others:

| Tool | What it does |
| --- | --- |
| `list_routes` | Every bucket the router supervises, with the paths it watches in each and whether this token can read it; the router's global filters (kinds, keywords, age); its routes with destinations, connectors and tool grants, description, goal, prose rules and trust; whether this token is the brain; the running cycle. |
| `claim_cycle` / `renew_cycle` / `release_cycle` | One run at a time on the ROUTER — the claim covers every bucket it supervises, and the answer lists them — under a lease; the cycle id the marks carry. |
...
| `sign_items` (`items` shape) | Called once per bucket, naming that bucket in `slug` under the one cycle: the record's route marks and router mark, with a status (`kept`, `processed`, `routed`, `pending`) and one link per route (`derive`, `merge`, `create`, `append`, `link`, `none`) naming the destination item and the finding ids the decision rested on. |
...
| `process_media` | Computes a record's findings now through the platform's processor. The supervising router only, and it **costs the workspace's credits**: use it only when the record has no findings. Your own model is free; the platform's processor is not. A record whose findings are current answers from cache at no cost. |

And two search the bucket itself, so a route can look before it writes:

| Tool | What it does |
| --- | --- |
| `search_records` | One bucket's records — titles, text and a capture's comments — searched for what already exists there, optionally scoped to a folder. One bucket per call and never more, so a router of several asks it once per bucket. Answers id, title, path, kind and a snippet. Free. |
