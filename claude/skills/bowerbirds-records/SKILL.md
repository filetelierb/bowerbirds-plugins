...
- `get_record_context` before deciding: the digest and signals say what a recording or a document is about, and a `part` says more. `process_media` computes a missing context at the workspace's expense — only for a record that has none; a router's own model is free, the platform's processor is platform credits.
