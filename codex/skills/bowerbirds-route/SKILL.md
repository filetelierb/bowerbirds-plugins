...
description: Run one routing cycle as a Bowerbirds router's external brain through the bowerbirds MCP server: claim_cycle takes the ROUTER's cycle, covering every bucket it supervises; list_routes names those buckets, the router's global filters and the routes; then per bucket the unsigned records and their findings, search_records before deciding, one dispatch per record per route (merge, derive, create, append, link or none), sign_items once per bucket and release_cycle once.
---

# Routing a Bowerbirds router
