...
description: Run one routing cycle over a Bowerbirds bucket as its external router through the bowerbirds MCP server: claim_cycle, list_routes, the unsigned records with the findings the media processor filed for them (get_record_context, get_media_chunk, and process_media only when a record has none - it costs credits), search the destination with search_records, one dispatch per record per route (merge, derive, create, append, link or none) through derive_record and the route's tools, then sign_items.
---
