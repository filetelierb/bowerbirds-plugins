...
A bucket has at most one router; a router may watch several buckets. A router of kind `external` names an agent token as its brain: a token minted with the `router` role, which may add and sign and nothing else. Holding that token, you run that ROUTER's cycle yourself — across every bucket it supervises — on your own model, with more tools (`/bowerbirds-route <bucket>` names any one of its buckets and runs the whole loop):

- `list_routes` answers the router supervising the bucket you name, and three lists. `buckets`: **every** bucket that router supervises, each with its `owner`, `slug`, the `paths` of it the router watches (absent means the whole bucket) and `thisToken`, which says whether your token can actually read it — a workspace token opens them all, a bucket-scoped token opens one and still sees the rest by name. `filters`: the ROUTER's global filters (`kinds`, `keywords`, `ageDays`), what it gathers at all in every bucket — the filters are the router's, and routes carry prose rules, not filter chips. `routes`: each route's `destinations` (workspace `paths` and `buckets` a derivation may land in; `external` targets inside a linked connector), its `connectors` with the tools it `enabled` and `disabled` by name, its `description`, `goal`, `rules`, and `trustedExternalCreate`. It says whether your token is the router's brain and which cycle is running. The routes are the allowlist the gate holds you to; a supervised bucket is not a destination by that fact alone.
- `claim_cycle` takes the ROUTER's cycle marker under a lease (default 10 minutes, at most 60) and answers the `cycleId` your marks carry, the router it reached and the buckets that one cycle covers — the router, not the bucket, is what claims, spends and signs. It is refused with the holder while another run is on the router, and that refusal holds every bucket the router supervises. `renew_cycle` extends it, `release_cycle` lets it go — always release when done, once for the whole router.
...
- `search_records` (`q`, `path?`, `limit?`) searches ONE bucket's records — titles, text and a capture's comments — for what already exists there, and answers id, title, path, kind and a snippet of the matching text. It never searches more than the bucket it is given, so a router of several buckets asks the question once per bucket it can read, naming each. `get_record` (`id`) reads one hit whole, its numbered comments included. Both free. Search the destination BEFORE you decide: that step is what turns a create into a merge or an update.
- `derive_record` files a NEW record from a source for a route: `sourceId`, `route`, `kind` (`split` or `augment`), `title`, `content`, `path` or `bucket` inside the route's destinations, `copyPayloads` (default true: the source's bytes are copied onto it), `cycle`. With `mergeInto` (a Package's id) the derived record joins that Package as its next scene; with `related` (record ids) a NEW Package is made of those records and the derived one — and `path` must then be the Package's, or the related records', directory. It carries `derivedFrom`, the route, its depth and origin chain; `bucket` defaults to the source record's own and the destination must be one the route's destinations allow, which need not be a bucket the router supervises. Landing in ANY bucket the router supervises it is signed `routed` at birth, so the router never gathers its own output. The gate judges it first (destinations, depth 3, five per source per day). The source is never written.
- `sign_items` has a second shape for the router: `items: [{id, status, links: [{route, kind, destination, connectorId, target, tool, bucket, path, findings}]}]` with the `cycle`. `slug` names the bucket those records live in — a record belongs to exactly one — so a cycle that read three buckets makes three `sign_items` calls under the same cycle, and naming the wrong bucket is refused with the router's whole supervised list. `kind` is one of `derive | merge | create | append | link | none`; `findings` names the finding ids that decision rested on (`f1`, `f-text-2`); `status` is `kept` (every route said none), `processed` (a route derived or merged), `routed` (dispatched externally, the record stays live) or `pending` (a route still owes a report), and it must agree with the links. Every link is judged by the gate against the route's allowlist, grant and trust; the answer says per record what was signed and what was refused. A record signed with a final status leaves the router's queue until it changes, and carries your plain signature too.
